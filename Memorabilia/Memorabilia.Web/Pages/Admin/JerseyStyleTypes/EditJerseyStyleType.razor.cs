...
{
    public partial class EditJerseyStyleType : ComponentBase
    {
        [Inject]
        ...
        {
            _viewModel = new SaveDomainViewModel(await QueryRouter.Send(new GetJerseyStyleType.Query(Id)).ConfigureAwait(false),
                                                 DomainTypeName,
                                                 ImagePath,
                                                 _navigationPath);
        }
        ...
        {
            await CommandRouter.Send(new SaveJerseyStyleType.Command(_viewModel)).ConfigureAwait(false);
        }
    }
}
