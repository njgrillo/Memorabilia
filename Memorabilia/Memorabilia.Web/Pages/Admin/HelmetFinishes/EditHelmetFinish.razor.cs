...
{
    public partial class EditHelmetFinish : ComponentBase
    {
        [Inject]
        ...
        private readonly string _navigationPath = $"{DomainTypeName.Replace(" ", "")}es";
        ...
        {
            _viewModel = new SaveDomainViewModel(await QueryRouter.Send(new GetHelmetFinish.Query(Id)).ConfigureAwait(false),
                                                 DomainTypeName,
                                                 ImagePath,
                                                 _navigationPath);
        }
        ...
        {
            await CommandRouter.Send(new SaveHelmetFinish.Command(_viewModel)).ConfigureAwait(false);
        }
    }
}
