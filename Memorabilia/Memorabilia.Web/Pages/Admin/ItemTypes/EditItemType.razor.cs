...
{
    public partial class EditItemType : ComponentBase
    {
        [Inject]
        ...
        {
            _viewModel = new SaveDomainViewModel(await QueryRouter.Send(new GetItemType.Query(Id)).ConfigureAwait(false),
                                                 DomainTypeName,
                                                 ImagePath,
                                                 _navigationPath);
        }
        ...
        {
            await CommandRouter.Send(new SaveItemType.Command(_viewModel)).ConfigureAwait(false);
        }
    }
}
