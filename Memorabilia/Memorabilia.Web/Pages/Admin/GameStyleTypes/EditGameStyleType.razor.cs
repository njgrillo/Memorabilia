...
{
    public partial class EditGameStyleType : ComponentBase
    {
        [Inject]
        ...
        {
            _viewModel = new SaveDomainViewModel(await QueryRouter.Send(new GetGameStyleType.Query(Id)).ConfigureAwait(false),
                                                 DomainTypeName,
                                                 ImagePath,
                                                 _navigationPath);
        }
        ...
        {
            await CommandRouter.Send(new SaveGameStyleType.Command(_viewModel)).ConfigureAwait(false);
        }
    }
}
