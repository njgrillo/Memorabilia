...
{
    public partial class EditJerseyType : ComponentBase
    {
        [Inject]
        ...
        {
            _viewModel = new SaveDomainViewModel(await QueryRouter.Send(new GetJerseyType.Query(Id)).ConfigureAwait(false),
                                                 DomainTypeName,
                                                 ImagePath,
                                                 _navigationPath);
        }
        ...
        {
            await CommandRouter.Send(new SaveJerseyType.Command(_viewModel)).ConfigureAwait(false);
        }
    }
}
