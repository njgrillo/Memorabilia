...
{
    public partial class EditAuthenticationCompany : ComponentBase
    {
        [Inject]
        ...
        {
            _viewModel = new SaveDomainViewModel(await QueryRouter.Send(new GetAuthenticationCompany.Query(Id)).ConfigureAwait(false),
                                                 DomainTypeName,
                                                 ImagePath,
                                                 NavigationPath);
        }
        ...
        {
            await CommandRouter.Send(new SaveAuthenticationCompany.Command(_viewModel)).ConfigureAwait(false);
        }
    }
}
