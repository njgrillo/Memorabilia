...
public class BasketballProfileModel : SportProfileModel
{
    public BasketballProfileModel() { }

    public BasketballProfileModel(Entity.Person person) : base(person) { }

    public override string ChampionshipSummaryDisplayText 
        => $"{Championships?.Count() ?? 0}x Finals Champion";
    ...
    public override string NameHeader
    {
        get
        {
            ...
            var hof = HallOfFames.SingleOrDefault(hof => hof.SportLeagueLevel == Constant.SportLeagueLevel.NationalBasketballAssociation);
